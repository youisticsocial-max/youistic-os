...
export default function NegotiationModal({
  ...
  const [originalValuation, setOriginalValuation] = useState("45000");
  const [negotiatedDiscount, setNegotiatedDiscount] = useState("8000");
  ...
  useEffect(() => {
    if (lead) {
      setProposalTitle(
        lead.service 
          ? `Revised Commercial Offer for ${lead.service}` 
          : `Customized Negotiated Business Offer`
      );
      if (lead.service) {
  ...
  const numericTotal = parseFloat(originalValuation) || 0;
  const numericDiscount = parseFloat(negotiatedDiscount) || 0;
  const netAmount = Math.max(0, numericTotal - numericDiscount);

  const proposalId = `YOS-NEG-${lead.id.substring(0, 5).toUpperCase()}`;
  ...
  const handlePrintDownload = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Negotiated_Proposal_${(lead.businessName || lead.clientName).replace(/\s+/g, '_')}_${proposalId}</title>
          <style>
    ...
    try {
      if (!(window as any).html2pdf) {
        await new Promise<void>((resolve, reject) => {
          ...
          script.onerror = () => reject(new Error("Failed to load PDF engine library"));
      ...
      const fileName = `Negotiation_Proposal_${(lead.businessName || lead.clientName).replace(/[^\w-]/g, '_')}_${proposalId}.pdf`;
      ...
      console.error("Direct PDF download error, resorting to print dialog:", err);
      handlePrintDownload();
    } finally {
  ...
  const handleSaveNegotiation = async (sendViaWhatsApp: boolean = true) => {
    ...
    try {
      ...
      await updateLeadStatus(lead.id, "NEGOTIATION", new Date(), formattedNote);
      setSuccess(true);

      if (sendViaWhatsApp) {
        const cleanedPhone = formatWhatsAppPhone(whatsappNumber);
        const message = `🤝 *REVISED COMMERCIAL OFFER / NEGOTIATION*

Dear *${lead.clientName}*,

Here is the revised commercial proposal for *${lead.businessName || lead.clientName}*:

📄 *Proposal Title:* ${proposalTitle}
🏷️ *Category:* ${packageType}
💰 *Original Total:* ₹${numericTotal.toLocaleString("en-IN")}
🏷️ *Special Negotiated Discount:* -₹${numericDiscount.toLocaleString("en-IN")}
✨ *Final Revised Valuation:* ₹${netAmount.toLocaleString("en-IN")}
...
*Youistic Enterprise Sales*`;
      ...
      console.error("Failed to save negotiation details:", error);
      ...
      <div style={{ width: "100%", maxWidth: "1050px", maxHeight: "92dvh", backgroundColor: "#0f111a", border: "1px solid #282d45", borderRadius: "20px", boxShadow: "0 24px 64px rgba(0,0,0,0.7)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ padding: "18px 24px", borderBottom: "1px solid #1f2338", backgroundColor: "#141726", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            <div style={{ width: "42px", height: "42px", borderRadius: "12px", background: "linear-gradient(135deg, #a855f7 0%, #6366f1 100%)", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 16px rgba(168,85,247,0.4)" }}>
            ...
            <div>
              <h2 style={{ fontSize: "18px", fontWeight: 700, color: "#f8fafc", margin: 0, display: "flex", alignItems: "center", gap: "8px" }}>
                Commercial Negotiation Generator
                <span style={{ fontSize: "11px", fontWeight: 600, color: "#c084fc", backgroundColor: "rgba(168,85,247,0.15)", padding: "3px 8px", borderRadius: "6px", border: "1px solid rgba(168,85,247,0.3)" }}>
                  Negotiated Proposal Engine
                </span>
              </h2>
              <p style={{ fontSize: "12px", color: "#94a3b8", margin: "2px 0 0" }}>
                Revise Scope, Adjust Valuation & Dispatch for <strong style={{ color: "#e2e8f0" }}>{lead.businessName || lead.clientName}</strong>
              </p>
            </div>
          </div>
            ...
            <div style={{ display: "flex", backgroundColor: "#0a0c14", padding: "3px", borderRadius: "10px", border: "1px solid #1f2338" }}>
              <button
                ...
                style={{
                  ...
                  border: "none",
                  cursor: "pointer",
                  backgroundColor: activeTab === "FORM" ? "#a855f7" : "transparent",
                  color: activeTab === "FORM" ? "white" : "#94a3b8",
                  transition: "all 0.2s"
                }}
              >
                <Edit3 size={14} /> Negotiation Form
              </button>
              <button
                ...
                style={{
                  ...
                  border: "none",
                  cursor: "pointer",
                  backgroundColor: activeTab === "PREVIEW" ? "#a855f7" : "transparent",
            ...
            <div style={{ width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                ...
                <div style={{ backgroundColor: "#141726", border: "1px solid #232840", borderRadius: "14px", padding: "16px" }}>
                  <h4 style={{ fontSize: "12px", fontWeight: 700, color: "#c084fc", textTransform: "uppercase", letterSpacing: "0.05em", margin: "0 0 12px" }}>
                ...
                <div>
                  <label style={{ display: "block", fontSize: "12px", fontWeight: 600, color: "#cbd5e1", marginBottom: "6px" }}>
                    Negotiated Proposal Document Title *
                  </label>
                  <input
                    type="text"
                    value={proposalTitle}
                ...
                <div>
                  <label style={{ display: "block", fontSize: "12px", fontWeight: 600, color: "#cbd5e1", marginBottom: "6px" }}>
                    Included Scope & Deliverables (Description) *
                  </label>
                  <textarea
                    rows={5}
                    value={deliverables}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                ...
                <div style={{ backgroundColor: "#141726", border: "1px solid #232840", borderRadius: "14px", padding: "16px", display: "flex", flexDirection: "column", gap: "14px" }}>
                  <h4 style={{ fontSize: "12px", fontWeight: 700, color: "#c084fc", textTransform: "uppercase", letterSpacing: "0.05em", margin: 0, display: "flex", alignItems: "center", gap: "6px" }}>
                    💰 Negotiated Quotation Calculator
                  </h4>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setOriginalValuation(e.target.value)}
                        style={{ width: "100%", backgroundColor: "#0b0d14", border: "1px solid #1f2338", borderRadius: "8px", padding: "8px 12px", fontSize: "14px", fontWeight: 700, color: "#e2e8f0", outline: "none" }}
                      />
                    </div>
                    <div>
                      <label style={{ display: "block", fontSize: "11px", color: "#fb923c", marginBottom: "4px" }}>
                  ...
                  <div style={{ borderTop: "1px solid #1f2338", paddingTop: "10px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: "12px", color: "#94a3b8" }}>Net Negotiated Valuation:</span>
                    <span style={{ fontSize: "20px", fontWeight: 800, color: "#34d399", fontFamily: "'Space Grotesk', sans-serif" }}>
                ...
                {/* Special Terms / Counter-Offer Notes */}
                <div>
                  <label style={{ display: "block", fontSize: "12px", fontWeight: 600, color: "#cbd5e1", marginBottom: "6px" }}>
                    Negotiation Notes / Special Conditions
                  </label>
                  <textarea
                    rows={3}
                    value={specialTerms}
                    onChange={(e) => setSpecialTerms(e.target.value)}
                    placeholder="e.g. Special 15% discount applied per client agreement on annual maintenance."
          ...
          <div style={{ position: "fixed", left: "-9999px", top: "-9999px", width: "750px", opacity: 0, pointerEvents: "none" }} aria-hidden="true">
            ...
            >
              ...
              <div style={{ backgroundColor: "#eef2ff", borderLeft: "4px solid #4f46e5", borderRadius: "6px", padding: "12px 16px", marginBottom: "24px" }}>
                ...
                <div style={{ fontSize: "12px", color: "#4338ca", marginTop: "2px", fontWeight: 500 }}>Category: {packageType} (Negotiated)</div>
              </div>
        ...
        <div style={{ padding: "16px 24px", borderTop: "1px solid #1f2338", backgroundColor: "#141726", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            ...
            >
              {isDownloadingPdf ? (
                ...
                <>
                  <Download size={16} color="#c084fc" /> Direct Download PDF (.pdf)
                </>
              )}
            </button>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <button
              type="button"
              onClick={() => handleSaveNegotiation(false)}
            ...
            >
              Save Only (Mark Negotiated)
            </button>

            <button
              type="button"
              onClick={() => handleSaveNegotiation(true)}
              disabled={isSubmitting || success}
              style={{
                display: "flex",
                alignItems: "center",
            ...
            >
              {success ? (
                <>
                  <CheckCircle2 size={16} /> Saved & Sent on WhatsApp!
                </>
              ) : isSubmitting ? (
                "Processing..."
              ) : (
